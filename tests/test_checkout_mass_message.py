...
class TestWizard(common.SingleTransactionCase):
    def setUp(self, *args, **kwargs):
        ...
        self.Checkout = self.env["tutorial.library.checkout"].with_user(admin_user)
        self.Wizard = self.env["tutorial.library.checkout.massmessage"].with_user(
            admin_user
        )
        a_member = self.env["tutorial.library.member"].create(
    ...
    def test_02_button_send_empty_body(self):
        ...
        Wizard0 = self.Wizard.with_context(active_ids=self.checkout0.ids)
        wizard0 = Wizard0.create({})
        with self.assertRaises(exceptions.UserError) as e:  # type: ignore
            wizard0.button_send()
